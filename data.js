  ...
  chambers: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: "sorrow 𓁿 — player-tier GAUGE (measures the colony's moral WEIGHT, not reach; NOT god-minted). LOOP: heavy conduct (killing) raises sorrow → offered DOWN at it housleth's sacrifice buildings → colony lightened = the moral turn (only the heavy can be made light; grief precedes repentance). housleth stays mints:nil (the altar PROCESSES sorrow, doesn't coin it — §3 law intact). VISIBILITY: conduct stays felt/unstated, sorrow is the ONE meter you SEE. Glyph 𓁿 (U+1307F eye-flowing-tears, drawn asset). Sorrow's GOD benched (until Affection/forsaking/tears-seam authored)", shards: 5 },
    ...
    },
    {
      ...
      items: [
        { name: "PLAYFIELD VISUAL (direction set) — the file-view is a DESKTOP-type surface: medium icons with names underneath (familiar OS icon-grid, out-competes Explorer on legibility) WITH ants running around among the icons. Files/folders = icons the colony lives on & HAULS (the co-op-haul spectacle on the grid). Fuses the ant-canvas (proto 001–003) with the file-view (004); the desktop IS the terrarium. Supersedes 004's gray-box list", shards: 4 },
