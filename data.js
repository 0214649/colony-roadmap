  ...
  chambers: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: "heft — the construction unit (LOCKED): a measure of MASS (build-side counterpart to sate). ONE unit underneath, surfacing as 6 SOLID TYPES across 3 BANDS on the organic→synthetic gradient (material = morality): ORGANIC chitin◗·fiber∥ | MINERAL stone▲·gem✦ | SYNTHETIC alloy◈·plastic▤. Start at 6, expand if earned. Binders (honey⬡ + resin/silk/mud/sap) join heft, not a heft type. WHY many = ambient-variety dopamine (always something to watch) + co-op hauls (many ants drag a big insect home together), NOT optimizer busywork; each type must read instantly when carried. Glyphs placeholder (art-pass owed)", shards: 6 },
        { name: "LIQUIDS — a class SEPARATE from heft (framed, not built): heft is solid & ant-HAULED, liquids FLOW and need their own transport infrastructure (pipes/channels/aqueducts) — the Factorio items-vs-fluids split, and where the elemental/magic flavour lives. Members (god-tied): water (aquatic/FROST axis) · fuel (the §4 wedge — burns → ENERGY) · lava (FIRE god + forbidden deep) · more TBD. Production tie: fuel → burned at synthetic tier → ENERGY → drives automation", shards: 2 },
